...
def getMenue_Mensen_weekly(locid, week):
	...
	file_Mensen = "mensa_menu_week"+str(currentWeek+week)+".pdf"
	try:
...
def getMenue_9b(day):
	...
	if day < 5:
		...
		try:
			WBurger =  string_format_9b('Wochenburger:((?s).*)\s\u20AC',ocr)
		except AttributeError:
			WBurger = string_format_9b('Wochenburger:((?s).*)Valle',ocr)
			
		out_obj_9b = np.array([[Mo,Di,Mi,Do,Fr][day],MBurger,WBurger])
...
def outFileWriter(outFile,flagFile, date, menue):
	...
	neunb_names = ['_Tagesmenü:_ \t', '_Monatsburger:_ \t', '_Wochenburger:_ \t']
